/** Chaînes de l'interface desktop, en français. Voir `en.ts` pour le contrat. */
const SETTINGS_TITLE = "Reqraft — Réglages";
const SECURE_STORAGE_UNAVAILABLE =
  "Le stockage sécurisé des identifiants n'est pas disponible sur ce système. Définissez {envName}, puis quittez complètement et relancez Reqraft.";

export const DESKTOP_FR = {
  "capsule.newReformulation": "nouvelle reformulation",
  "capsule.selectionFrom": "sélection · {app}",
  "capsule.placeholder": "Qu'est-ce que tu veux mieux formuler ?",
  "capsule.reformulate": "reformuler",
  "capsule.close": "fermer",
  "capsule.readingSelection": "Lecture de la sélection…",
  "capsule.analysingIntent": "Analyse de l'intention…",
  "capsule.preparing": "Préparation…",
  "capsule.detectedPreparing": "{profile} détecté · préparation…",
  "capsule.receiving": "réception…",
  "capsule.switchApps": "tu peux changer d'app",
  "capsule.before": "avant",
  "capsule.profile": "profil",
  "capsule.autoDetected": "détecté automatiquement",
  "capsule.interrupt": "interrompre",
  "capsule.replace": "remplacer",
  "capsule.compare": "comparer",
  "capsule.compareTitle": "Comparaison épinglée (⌘D) · ⌥ maintenu pour un coup d'œil",
  "capsule.copy": "copier",
  "capsule.rerun": "relancer",
  "capsule.correctOnly": "Corriger",
  "capsule.correctOnlyTitle":
    "Relancer depuis le texte de départ avec le profil clean au niveau minimal",
  "capsule.textPreserved": "✓ texte conservé",
  "capsule.textPreservedDetail": "le texte de départ est resté inchangé",
  "capsule.editedVerdict": "modifié · à relire",
  "capsule.sourceEditedVerdict": "source modifiée · à relancer",
  "capsule.sourceEditedDetail": "le résultat vient du texte de départ précédent",
  "capsule.editedDetail": "les vérifications portaient sur la réponse initiale",
  "capsule.informativeSignals": "signaux informatifs",
  "capsule.checkResult": "vérifie que le résultat conserve ton intention",
  "capsule.level": "niveau",
  "capsule.error": "Erreur",
  "capsule.escToClose": "esc pour fermer",
  "capsule.levelTitle": "Niveau de reformulation (cliquer pour changer)",
  "capsule.levelCycleTitle": "⇥ niveau suivant · ⇧⇥ niveau précédent",
  "capsule.changeProfile": "Changer de profil",
  "capsule.analysing": "analyse…",
  "capsule.copied": "Résultat copié.",
  "clipboard.copyFailed": "Le résultat n’a pas pu être copié.",
  "capsule.editLabel": "Résultat — modifiable avant de remplacer ou de copier",
  "capsule.editEmpty": "Le résultat est vide : écris quelque chose avant de l'appliquer.",
  "capsule.editPromptLabel": "Prompt de départ — modifiable avant de relancer",
  "capsule.promptEmpty": "Le prompt de départ est vide : écris quelque chose avant de relancer.",
  "capsule.replaceUnavailable": "Remplacement indisponible — résultat copié, ⌘V pour coller.",
  "capsule.replaceFailed": "Le remplacement a échoué — le résultat reste affiché.",
  "capsule.qualityGood": "✓ aucune alerte",
  "capsule.qualityReview": "à relire",
  "capsule.qualityRisky": "risqué",
  "capsule.expansionDetected": "! expansion détectée",
  "capsule.expansionDetail": "fonctionnalités non demandées",
  "capsule.noInvention": "aucun écart repéré par les vérifications locales",
  "capsule.lowerLevel": "baisser le niveau",

  "settings.title": SETTINGS_TITLE,
  "settings.ready": "prêt",
  "settings.tagline": "Shape the request. Keep the intent.",
  "settings.nav.profiles": "Profils",
  "settings.nav.providers": "Providers",
  "settings.nav.models": "Modèles",
  "settings.nav.preferences": "Réglages",
  "settings.nav.updates": "Mises à jour",
  "settings.nav.diagnostic": "Diagnostic",
  "settings.profiles.detail": "Style de reformulation et profils locaux.",
  "settings.providers.detail": "Vos clés et vos endpoints, sans jamais afficher une clé.",
  "settings.models.detail": "Provider, modèle et niveau utilisés par défaut.",
  "settings.models.routing": "Provider et modèle",
  "settings.models.rewriting": "Reformulation",
  "settings.preferences.detail": "Raccourcis globaux, permissions et préférences desktop.",
  "settings.preferences.shortcutsGroup": "Raccourcis globaux",
  "settings.preferences.interfaceGroup": "Interface",
  "settings.preferences.generationGroup": "Génération",
  "settings.preferences.systemGroup": "Système",
  "settings.updates.detail": "Version installée et disponibilité d'une nouvelle version.",
  "settings.updates.current": "Version installée",
  "settings.updates.currentDetail": "La version de l'application actuellement ouverte.",
  "settings.updates.latest": "Dernière version",
  "settings.updates.latestDetail": "La version la plus récente publiée pour Reqraft.",
  "settings.updates.versionGroup": "Versions",
  "settings.updates.notChecked": "Aucune vérification effectuée.",
  "settings.updates.checking": "Vérification…",
  "settings.updates.upToDate": "Reqraft est à jour.",
  "settings.updates.available": "Reqraft {version} est disponible.",
  "settings.updates.error": "Impossible de vérifier les mises à jour. Réessayez plus tard.",
  "settings.updates.check": "Vérifier maintenant",
  "settings.updates.download": "Télécharger",
  "settings.updates.startupNote":
    "Reqraft vérifie automatiquement au démarrage et ouvre la page de téléchargement dans votre navigateur.",
  "settings.diagnostic.detail": "Vérifications locales et rapport sans secret.",
  "settings.diagnosticChecks": "Vérifications système",
  "settings.diagnosticOk": "OK",
  "settings.diagnosticAttention": "À vérifier",
  "settings.diagnosticReady": "Lancez le diagnostic pour vérifier cette installation.",
  "settings.diagnosticAllClear": "Toutes les vérifications sont validées.",
  "settings.diagnosticIssue": "Une vérification demande votre attention.",
  "settings.diagnosticIssues": "{count} vérifications demandent votre attention.",
  "settings.diagnosticConfigFile": "Fichier de configuration",
  "settings.diagnosticDefaults": "Configuration par défaut",
  "settings.diagnosticAccessibility": "Permission d’accessibilité",
  "settings.diagnosticAutomation": "Permission d’automatisation",
  "settings.diagnosticReplacement": "Remplacement du texte",
  "settings.diagnosticRejectedShortcuts": "Raccourcis refusés",
  "settings.diagnosticShortcutConflicts": "Conflits de raccourcis",
  "settings.diagnosticShortcutState": "État des raccourcis",
  "settings.diagnosticProvider": "Provider {provider}",
  "settings.diagnosticPassed": "Contrôles réussis ({count})",
  "settings.diagnosticFailed": "Le diagnostic n'a pas pu s'exécuter. Réessayez.",
  "settings.remedy.grantAccessibility":
    "Reqraft a besoin de l'Accessibilité pour taper le remplacement. Autorisez-la, ou ouvrez Réglages système › Confidentialité et sécurité › Accessibilité si l'invite ne s'affiche plus.",
  "settings.remedy.grantAutomation":
    "macOS demande l'Automatisation au premier remplacement, et ne la redemande jamais. Cochez Reqraft sous Événements système dans Confidentialité et sécurité › Automatisation.",
  "settings.remedy.grantPermissions":
    "Le remplacement devient disponible dès que l'autorisation manquante est accordée. En attendant, accepter un résultat le copie, prêt à être collé avec ⌘V.",
  "settings.remedy.waylandFloor":
    "Wayland refuse l'injection de frappe par conception. Rien à autoriser : ⏎ copie le résultat, et ⌘V le colle où vous voulez.",
  "settings.remedy.pickShortcut":
    "Aucune combinaison n'a pu être enregistrée pour cette commande. Choisissez-en une autre dans Réglages.",
  "settings.remedy.freeShortcut":
    "Une autre application détient cette combinaison. Libérez-la de son côté, ou choisissez-en une autre dans Réglages.",
  "settings.remedy.resolveShortcutConflict":
    "Deux commandes Reqraft demandent la même combinaison. Changez-en une dans Réglages.",
  "settings.remedy.resumeShortcuts":
    "Les raccourcis globaux sont suspendus depuis la barre de menus. Reprenez-les pour retrouver les trois commandes.",
  "settings.remedy.configureProvider":
    "Ce fournisseur n'a pas de configuration utilisable. Ajoutez ou remplacez sa clé dans Providers.",
  "settings.remedy.openSystemSettings": "Ouvrir les Réglages système",
  "settings.remedy.openShortcuts": "Ouvrir Réglages",
  "settings.remedy.openProviders": "Providers",
  "settings.remedy.resume": "Reprendre",
  "settings.remedy.systemSettingsOpened":
    "Réglages système ouverts — le diagnostic sera relancé à votre retour.",
  "settings.remedy.permissionRequested":
    "Autorisation demandée. Si aucune invite ne s'affiche, macOS a déjà une réponse : ouvrez les Réglages système.",
  "settings.remedy.shortcutsResumed": "Raccourcis globaux repris.",
  "settings.remedy.actionFailed": "Cette action n'a pas pu être menée à bien.",
  "settings.context": "Contexte",
  "settings.context.provider": "provider",
  "settings.context.model": "modèle",
  "settings.context.profile": "profil",
  "settings.context.level": "niveau",
  "settings.footer": "Local-first · aucun prompt stocké",
  "settings.loadingConfig": "Lecture des réglages",
  "settings.builtinProviders": "Fournisseurs intégrés",
  "settings.compatibleProviders": "Fournisseurs compatibles OpenAI",
  "settings.addProvider": "Ajouter un fournisseur",
  "settings.addKey": "Ajouter une clé",
  "settings.replaceKey": "Remplacer la clé",
  "settings.remove": "Retirer",
  "settings.delete": "Supprimer",
  "settings.deleteForever": "Supprimer définitivement",
  "settings.confirm": "Confirmer",
  "settings.cancel": "Annuler",
  "settings.save": "Enregistrer",
  "settings.saving": "Enregistrement…",
  "settings.verify": "Vérifier",
  "settings.providerTest": "Tester",
  "settings.providerTesting": "Test en cours…",
  "settings.providerTestOk": "Configuration locale valide.",
  "settings.providerTestMissing": "Configuration incomplète : {list}.",
  "settings.providerTestMissingUnknown": "Configuration incomplète.",
  "settings.providerTestInvalid": "Configuration refusée par le fournisseur.",
  "settings.providerTestUnreachable": "Fournisseur injoignable.",
  "settings.providerTestError": "Le test n'a pas abouti.",
  "settings.providerTestNote":
    "Un test vérifie ce qui est configuré localement ; aucune requête n'est envoyée au fournisseur.",
  "settings.edit": "Modifier",
  "settings.pasteKey": "Collez votre clé",
  "settings.noKeyStored": "Aucune clé enregistrée.",
  "settings.secureStorageUnavailable": SECURE_STORAGE_UNAVAILABLE,
  "settings.keyInKeychain": "Clé enregistrée dans votre trousseau.",
  "settings.keyNotNeeded": "Aucune clé nécessaire.",
  "settings.keyFromEnv": "Clé lue dans {envName}.",
  "settings.defaultProvider": "Provider par défaut",
  "settings.defaultProviderDetail": "Utilisé par la capsule et le popover.",
  "settings.defaultBadge": "Par défaut",
  "settings.defaultBadgeTitle": "Le fournisseur utilisé par la capsule et le popover.",
  "settings.defaultBadgeEndpointTitle":
    "Premier endpoint déclaré : celui utilisé pour les requêtes compatibles OpenAI.",
  "settings.defaultModel": "Modèle par défaut",
  "settings.defaultModelDetail": "Les modèles pris en charge par ce provider, ou le vôtre.",
  "settings.modelId": "Identifiant du modèle",
  "settings.modelIdDetail": "Envoyé tel quel au provider choisi.",
  "settings.otherModel": "Autre identifiant…",
  "settings.refresh": "Actualiser",
  "settings.refreshModels": "Actualiser le catalogue de modèles",
  "settings.modelCatalogLoading": "Chargement du catalogue du provider…",
  "settings.modelCatalogReady": "{count} modèles disponibles.",
  "settings.modelCatalogReadyTruncated": "{count} premiers modèles disponibles ; liste raccourcie.",
  "settings.modelCatalogEmpty":
    "Le provider a renvoyé un catalogue vide. Saisissez un identifiant.",
  "settings.modelCatalogUnsupported":
    "Ce provider n’expose pas de catalogue. Saisissez un identifiant.",
  "settings.modelCatalogNoEndpoint": "Ajoutez un endpoint compatible pour charger son catalogue.",
  "settings.modelCatalogMissing":
    "Configuration manquante : {list}. Saisissez un identifiant si nécessaire.",
  "settings.modelCatalogMissingUnknown": "La configuration du provider est incomplète.",
  "settings.modelCatalogError": "Catalogue indisponible. L’identifiant actuel est conservé.",
  "settings.defaultLevel": "Niveau par défaut",
  "settings.defaultLevelDetail": "S'applique quand le profil ne force aucun niveau.",
  "settings.macosPermissions": "Permissions macOS",
  "settings.permissionsGranted": "Accessibilité et Automatisation accordées.",
  "settings.permissionsNeeded": "Requises pour la capture et le remplacement.",
  "settings.permissionsOk": "Accordées",
  "settings.permissionsChecking": "Vérification…",
  "settings.allow": "Autoriser…",
  "settings.rerunDiagnostic": "Relancer le diagnostic",
  "settings.diagnosticRunning": "Diagnostic en cours…",
  "settings.copyReport": "Copier le rapport",
  "settings.copyingReport": "Copie du rapport…",
  "settings.reportCopied": "Rapport copié — à coller dans une issue GitHub.",
  "settings.reportCopyFailed": "Le rapport n'a pas pu être copié.",
  "settings.captureShortcut": "Reformuler la sélection",
  "settings.inputShortcut": "Ouvrir sans sélection",
  "settings.popoverShortcut": "Ouvrir le popover",
  "settings.identifier": "Identifiant",
  "settings.providerFormCreate": "Ajouter un provider compatible OpenAI.",
  "settings.providerFormEdit": "Modifier ce provider compatible OpenAI.",
  "settings.identifierTaken": "Cet identifiant est déjà pris.",
  "settings.identifierFormat":
    "L'identifiant n'accepte que des minuscules, des chiffres et des tirets.",
  "settings.identifierMissing": "Donnez un identifiant à ce fournisseur.",
  "settings.baseUrlScheme": "L'URL de base doit commencer par http:// ou https://.",
  "settings.automatic": "Automatique",
  "settings.noCustomProvider":
    "Aucun fournisseur personnalisé. Ajoutez-en un pour appeler un serveur local ou une passerelle compatible.",
  "settings.keysNote":
    "Quand le système le permet, les clés sont vérifiées auprès du fournisseur puis rangées dans son trousseau. Elles ne sont jamais écrites dans votre configuration, ni affichées ici.",
  "settings.shortcutRestart":
    "Les changements s'appliquent immédiatement. Si une combinaison est indisponible, Reqraft active un repli.",
  "settings.shortcutsReset": "Réinitialiser",
  "settings.shortcutsRetest": "Retester",
  "settings.replaceEnvInApp":
    "Cette clé vient de l'environnement de lancement. Remplacez-la ici pour que l'application utilise plutôt le trousseau.",
  "profiles.intro":
    "Le profil sélectionné est celui que la capsule utilise. Un profil intégré se duplique pour obtenir une copie modifiable.",
  "profiles.empty":
    "Aucun profil local. « Nouveau profil local » en crée un, ou dupliquez un profil intégré.",
  "profiles.new": "Nouveau profil local",
  "profiles.formCreateTitle": "Nouveau profil local",
  "profiles.formUpdateTitle": "Modifier « {id} »",
  "profiles.formDuplicateTitle": "Dupliquer « {id} »",
  "profiles.use": "Utiliser ce profil",
  "profiles.duplicate": "Dupliquer vers un nouveau profil local",
  "profiles.export": "Exporter en JSON",
  "profiles.confirmDelete": "Confirmer la suppression",
  "profiles.builtinNotEditable": "Un profil intégré n'est pas modifiable",
  "profiles.builtinNotDeletable": "Un profil intégré n'est pas supprimable",
  "profiles.builtin": "Intégré",
  "profiles.name": "Nom",
  "profiles.description": "Description",
  "profiles.base": "Base",
  "profiles.instructions": "Instructions",
  "profiles.defaultLevel": "Niveau par défaut",
  "profiles.duplicateNote":
    "La copie reprend la description, le niveau et les instructions du profil source. Choisissez seulement son identifiant local et son nom.",
  "profiles.nameRequired": "Le nom est obligatoire.",
  "profiles.idRequired": "L'identifiant est obligatoire.",
  "profiles.idInvalid": "Identifiant invalide : minuscules, chiffres et tirets uniquement.",
  "profiles.idTaken": "L'identifiant « {id} » est déjà pris.",
  "profiles.descriptionRequired": "La description est obligatoire.",
  "profiles.instructionsRequired": "Les instructions sont obligatoires.",
  "onboarding.title": "Configurons Reqraft",
  "onboarding.lede": "Quelques choix suffisent. Vous les retrouverez tous dans les réglages.",
  "onboarding.badge": "configuration",
  "onboarding.loading": "Lecture de votre configuration…",
  "onboarding.provider": "Fournisseur",
  "onboarding.providerDetail": "Le service qui reformulera vos demandes.",
  "onboarding.internalId": "Identifiant interne",
  "onboarding.internalIdDetail": "Le nom court sous lequel ce fournisseur est enregistré.",
  "onboarding.baseUrl": "URL de base",
  "onboarding.baseUrlDetail": "L'adresse de l'API compatible OpenAI à appeler.",
  "onboarding.apiKey": "Clé API {provider}",
  "onboarding.apiKeyDetail":
    "Vérifiée puis rangée dans le trousseau de votre système. Elle n'est jamais écrite dans votre fichier de configuration.",
  "onboarding.verifyAndSave": "Vérifier et enregistrer",
  "onboarding.model": "Modèle",
  "onboarding.modelDetail": "Ce que le fournisseur exécutera.",
  "onboarding.modelPlaceholder": "identifiant du modèle",
  "onboarding.defaultProfile": "Profil par défaut",
  "onboarding.defaultProfileDetail": "Le style appliqué quand vous n'en choisissez pas un autre.",
  "onboarding.level": "Niveau de réécriture",
  "onboarding.levelDetail": "À quel point Reqraft retravaille ce que vous écrivez.",
  "onboarding.levelMinimal": "Minimale — corrige la forme, touche à peu",
  "onboarding.levelStandard": "Standard — reformule et structure",
  "onboarding.levelComplete": "Complète — restructure et détaille",
  "onboarding.finish": "Terminer la configuration",
  "onboarding.pickProvider": "Choisissez un fournisseur.",
  "onboarding.idRequired": "Donnez un identifiant à votre fournisseur (par exemple « local »).",
  "onboarding.idFormat":
    "L'identifiant du fournisseur n'accepte que des minuscules, des chiffres et des tirets.",
  "onboarding.modelRequired": "Indiquez le modèle à utiliser.",
  "onboarding.keyRequired":
    "Enregistrez une clé API pour {provider} : sans elle, l'application ne peut rien envoyer.",
  "onboarding.ready": "Tout est prêt : vous pouvez terminer.",
  "onboarding.keyFromEnv": "Clé détectée dans {envName}.",
  "onboarding.keyInKeychain": "Clé trouvée dans le trousseau de votre système.",
  "onboarding.providerDeclared": "Fournisseur déclaré dans votre configuration.",
  "onboarding.keyNotNeeded": "Aucune clé nécessaire.",
  "onboarding.keyNotNeededProvider": "Aucune clé nécessaire pour ce fournisseur.",
  "onboarding.noKeyYet": "Aucune clé enregistrée pour l'instant.",
  "onboarding.keySaved": "Clé {provider} vérifiée et enregistrée dans votre trousseau.",
  "onboarding.savedIncomplete":
    "Configuration enregistrée, mais incomplète : il manque encore une clé utilisable.",
  "onboarding.unreadable": "Configuration illisible : {reason}",
  "popover.placeholder": "Écris, ou ⌘V pour coller…",
  "popover.promptLabel": "Prompt de départ — modifiable avant de relancer",
  "popover.receiving": "Réception…",
  "popover.lastResult": "Dernier résultat",
  "popover.resultLabel": "Résultat — modifiable avant de copier",
  "popover.resultEmpty": "Le résultat est vide : écris quelque chose avant de le copier.",
  "popover.copy": "copier",
  "popover.copied": "Résultat copié.",
  "popover.settings": "Réglages…",
  "popover.browse": "parcourir",
  "popover.choose": "choisir",
  "popover.back": "retour",
  "picker.title": "Profils",
  "picker.available": "{count} disponibles",
  "picker.search": "Rechercher un profil…",
  "picker.noMatch": "Aucun profil ne correspond.",
  "picker.manage": "Gérer les profils…",
  "picker.dialogLabel": "Choisir un profil",
  "picker.backToInput": "Revenir à la saisie",
  "picker.groupAuto": "Automatique",
  "picker.groupBuiltin": "Intégrés",
  "picker.groupLocal": "Mes profils",
  "picker.active": "actif",
  "shortcut.space": "Espace",
  "profiles.noneLevel": "aucune",
  "profiles.notEditableTag": "non modifiable",
  "main.traySettings": "Réglages…",
  "main.traySuspendShortcuts": "Suspendre les raccourcis globaux",
  "main.trayQuit": "Quitter Reqraft",
  "main.trayIdle": "Reqraft — prêt",
  "main.trayBusy": "Reqraft — reformulation en cours…",
  "main.trayError": "Reqraft — une erreur est survenue",
  "main.updateTitle": "Mise à jour Reqraft disponible",
  "main.updateBody": "La version {version} peut être téléchargée.",
  "main.trayUpdateAvailable": "Télécharger Reqraft {version}",
  "main.trayShortcutsSuspendedTooltip": "Reqraft — raccourcis globaux suspendus",
  "main.settingsTitle": SETTINGS_TITLE,
  "main.onboardingTitle": "Reqraft — Configuration",
  "main.profileFileType": "Profil Reqraft",
  "main.permissionsGranted": "Accessibilité et Automatisation accordées.",
  "main.permissionsAccessibilityMissing":
    "Automatisation accordée, Accessibilité refusée — les frappes ne partiront pas.",
  "main.permissionsAutomationMissing": "Accessibilité accordée, Automatisation refusée.",
  "main.permissionsNoneGranted":
    "Ni Accessibilité ni Automatisation accordées : la capsule fonctionne en saisie manuelle.",
  "main.permissionsWayland": "Wayland bloque l'injection de frappes.",
  "main.permissionsWaylandDetail":
    "Wayland bloque l'injection de frappes par conception : la capsule fonctionne, mais ⏎ copie au lieu de remplacer.",
  "main.permissionsNotNeeded": "Aucune permission requise sur cette plateforme.",
  "main.permissionsPending": "Permissions pas encore vérifiées.",
  "main.captureClipboardNotText": "presse-papiers non textuel",
  "main.captureNoSelection": "aucune sélection",
  "main.captureAccessibilityDenied":
    "accès Accessibilité refusé : autorisez Reqraft dans Réglages Système › Confidentialité et sécurité › Accessibilité",
  "main.captureAutomationDenied":
    "accès Automatisation refusé : autorisez Reqraft à contrôler « System Events » dans Réglages Système › Confidentialité et sécurité › Automatisation",
  "main.captureFailed": "capture impossible : {detail}",
  "main.replaceSourceInactive": "application source non réactivée",
  "main.replaceSourceUnknown": "application source inconnue",
  "main.doctorConfigIncomplete": "configuration incomplète",
  "main.doctorValidationError": "erreur de validation",
  "main.doctorPermissionGranted": "accordée",
  "main.doctorNoRejection": "aucun refus",
  "main.doctorRejectedBySystem": "refusés par le système : {list}",
  "main.doctorNoConflict": "aucun doublon",
  "main.doctorConflictingShortcuts": "déjà utilisés par une autre commande Reqraft : {list}",
  "main.doctorShortcutsActive": "raccourcis globaux actifs",
  "main.doctorShortcutsSuspended": "raccourcis globaux suspendus depuis la barre de menus",
  "main.doctorNoShortcut": "aucun raccourci actif",
  "main.autoProfileName": "Auto",
  "main.autoProfileSummary": "Détection locale du profil",
  "main.autoProfileDescription": "Reqraft laisse le modèle choisir le profil adapté au texte.",
  "main.errorProviderNotStorable":
    "Le fournisseur {provider} ne prend pas de clé enregistrable dans le trousseau.",
  "main.errorProviderNotSelectable":
    "Le fournisseur {provider} ne peut pas être choisi à la configuration.",
  "main.errorProviderNoStoredKey":
    "Le fournisseur {provider} n'a pas de clé enregistrée dans le trousseau.",
  "main.errorProviderUnknown": "Aucun fournisseur personnalisé nommé {id}.",
  "main.errorProfileBuiltin":
    "« {id} » est un profil intégré : il n'est ni modifiable ni supprimable. Dupliquez-le pour en obtenir une copie.",
  "main.errorProfileIsDefault":
    "« {id} » est le profil par défaut : choisissez-en un autre avant de le supprimer.",
  "common.recommendedSuffix": " — recommandé",
  "settings.endpointNoKey": "Sans clé.",
  "settings.endpointIdDetail": "Minuscules, chiffres et tirets.",
  "settings.endpointNameLabel": "Nom affiché",
  "settings.endpointNameDetail": "Facultatif.",
  "settings.endpointBaseUrlLabel": "URL de base",
  "settings.endpointBaseUrlDetail": "L'API compatible OpenAI à appeler.",
  "settings.endpointKeyEnvLabel": "Variable de clé",
  "settings.endpointKeyEnvDetail": "Facultatif : le nom de la variable d'environnement à lire.",
  "settings.captureShortcutDetail": "Capsule ancrée au curseur",
  "settings.inputShortcutDetail": "Capsule centrée, saisie libre",
  "settings.popoverShortcutDetail": "Panneau de la barre de menus, sans passer par l'icône",
  "settings.shortcutsTaken":
    "Raccourcis déjà pris par une autre application : {list}. Un repli a été enregistré — modifiez le raccourci concurrent pour utiliser votre choix préféré.",
  "settings.shortcutsNone":
    "Aucun raccourci global disponible. Reqraft ne peut pas se déclencher au clavier — libérez-en un, puis relancez l'application.",
  "settings.shortcutUnavailable": "Choix non disponible : {accelerator} est actif à la place.",
  "settings.shortcutsConflicting":
    "Raccourcis demandés deux fois par Reqraft : {list}. Une seule commande peut répondre à une combinaison — un repli a été enregistré pour l'autre, choisissez-lui une combinaison distincte.",
  "settings.shortcutsSuspended":
    "Les raccourcis globaux sont suspendus. Réactivez-les depuis l'icône Reqraft de la barre de menus.",
  "settings.language": "Langue de l'interface",
  "settings.languageDetail": "Reqraft redémarre automatiquement quand ce choix change.",
  "settings.welcomeTour": "Présentation de Reqraft",
  "settings.welcomeTourDetail": "Revoir les principaux usages et les fonctions disponibles.",
  "settings.welcomeTourReplay": "Revoir",
  "settings.languageAuto": "Langue du système",
  "settings.languageEn": "English",
  "settings.languageFr": "Français",
  "settings.fidelity": "Fidélité",
  "settings.fidelityDetail": "Jusqu'où la réécriture peut s'éloigner de ce que vous avez écrit.",
  "settings.fidelityStrict": "Stricte - garder la formulation",
  "settings.fidelityBalanced": "Équilibrée - recommandée",
  "settings.fidelityPermissive": "Permissive - réécrire librement",
  "settings.outputLanguage": "Langue des prompts",
  "settings.outputLanguageDetail": "La langue dans laquelle les prompts sont rédigés.",
  "settings.outputLanguageAuto": "Suivre le texte d'entrée",
  "settings.outputLanguageCustom": "Choisir une langue",
  "settings.outputLanguageCustomLabel": "Langue de sortie à utiliser",
  "settings.outputLanguagePlaceholder": "fr-FR",
  "settings.outputLanguageInvalid": "Indiquez une langue, par exemple français ou fr-FR.",
  "settings.timeout": "Délai de réponse",
  "settings.timeoutDetail": "Le temps laissé au fournisseur avant d'abandonner l'exécution.",
  "settings.timeoutUnit": "secondes",
  "settings.timeoutInvalid": "Indiquez une durée en secondes supérieure à 0.",
  "settings.maxOutputTokens": "Tokens de sortie maximum",
  "settings.maxOutputTokensDetail": "Laissez vide pour laisser le fournisseur décider.",
  "settings.maxOutputTokensAuto": "Automatique",
  "settings.maxOutputTokensInvalid": "Indiquez un entier supérieur à 0, ou laissez vide.",
  "profiles.tooltipInUse": "Profil utilisé par la capsule",
  "profiles.tooltipAutoNoDuplicate": "Le profil automatique ne se duplique pas",
  "profiles.tooltipAutoNoExport": "Le profil automatique ne s'exporte pas",
  "onboarding.language": "Langue",
  "onboarding.languageDetail": "La langue dans laquelle Reqraft vous parle.",
  "onboarding.tour.badge": "découverte",
  "onboarding.tour.step": "{current} sur {total}",
  "onboarding.tour.progress": "Progression de la présentation",
  "onboarding.tour.goTo": "Aller à l'écran {step}",
  "onboarding.tour.skip": "Passer",
  "onboarding.tour.back": "Retour",
  "onboarding.tour.next": "Continuer",
  "onboarding.tour.configure": "Configurer Reqraft",
  "onboarding.tour.replay": "Rejouer la démonstration",
  "onboarding.tour.mail.title": "Des e-mails clairs sans quitter Mail",
  "onboarding.tour.mail.body":
    "Sélectionnez un brouillon, lancez Reqraft et remplacez-le par une version prête à envoyer.",
  "onboarding.tour.mail.compose": "Nouveau message",
  "onboarding.tour.mail.draft": "Brouillon",
  "onboarding.tour.mail.to": "À",
  "onboarding.tour.mail.subject": "Objet",
  "onboarding.tour.mail.subjectValue": "Suivi du dossier",
  "onboarding.tour.mail.hello": "Bonjour,",
  "onboarding.tour.mail.original": "salut, est-ce que le dossier est prêt merci",
  "onboarding.tour.mail.rewritten":
    "Bonjour, pourriez-vous me confirmer que le dossier est prêt ? Merci.",
  "onboarding.tour.mail.signoff": "Bonne journée.",
  "onboarding.tour.chat.title": "Préparez vos prompts avant de les envoyer",
  "onboarding.tour.chat.body":
    "Dans ChatGPT ou un autre chat, Reqraft transforme une idée brute en demande structurée et vérifiable.",
  "onboarding.tour.chat.newChat": "Nouveau chat",
  "onboarding.tour.chat.workspace": "Espace IA",
  "onboarding.tour.chat.recent": "Récents",
  "onboarding.tour.chat.threadOne": "Plan de lancement",
  "onboarding.tour.chat.threadTwo": "Textes d'accueil",
  "onboarding.tour.chat.availableModels": "Assistants IA disponibles",
  "onboarding.tour.chat.placeholder": "Envoyer un message à ChatGPT",
  "onboarding.tour.chat.assistant": "Que puis-je faire pour vous ?",
  "onboarding.tour.chat.original": "fais moi un plan pour lancer le site sans rien oublier",
  "onboarding.tour.chat.rewritten":
    "Crée un plan de lancement du site avec prérequis, étapes, responsables et critères de validation.",
  "onboarding.tour.code.title": "Conservez chaque contrainte technique",
  "onboarding.tour.code.body":
    "Le profil code préserve les composants, chemins et termes importants au lieu de les simplifier.",
  "onboarding.tour.code.original": "{/* add a red button here but do not touch navigation */}",
  "onboarding.tour.code.rewritten":
    "Ajoute un bouton rouge avant <Navigation />, sans modifier ce composant ni son comportement.",
  "onboarding.tour.profiles.title": "Un profil pour chaque manière d'écrire",
  "onboarding.tour.profiles.body":
    "Laissez Reqraft détecter le contexte, choisissez un profil intégré ou ajoutez le vôtre.",
  "onboarding.tour.profiles.available": "Profils disponibles",
  "onboarding.tour.profiles.search": "Rechercher un profil…",
  "onboarding.tour.profiles.detects": "détecte le contexte",
  "onboarding.tour.profiles.clarify": "clarifier",
  "onboarding.tour.profiles.agents": "agents de code",
  "onboarding.tour.profiles.add": "Ajouter un profil personnalisé",
  "onboarding.tour.providers.title": "Choisissez votre fournisseur",
  "onboarding.tour.providers.body":
    "Configurez un fournisseur intégré ou compatible, puis changez-en sans modifier votre workflow.",
  "onboarding.tour.providers.builtIn": "Fournisseurs intégrés",
  "onboarding.tour.providers.ecosystemTitle": "Vos IA, une seule interface",
  "onboarding.tour.providers.ecosystemDetail":
    "Fournisseurs natifs et endpoints compatibles OpenAI",
  "onboarding.tour.providers.addKey": "Ajouter une clé",
  "onboarding.tour.providers.compatible": "Ajouter un fournisseur compatible OpenAI",
  "onboarding.tour.providers.keychain": "Les clés sont protégées par le trousseau macOS",
  "onboarding.tour.capture.editing": "modification",
  "onboarding.tour.privacy.title": "Vos demandes restent éphémères",
  "onboarding.tour.privacy.body":
    "Reqraft ne conserve pas vos prompts. Vous choisissez le fournisseur et les clés restent dans le trousseau du système.",
  "onboarding.tour.privacy.private": "privé",
  "onboarding.tour.privacy.example":
    "Clarifie cette demande sans ajouter de fonctionnalité ni perdre de contrainte.",
  "onboarding.tour.privacy.telemetry": "Télémétrie désactivée · contenu non conservé",
  "onboarding.tour.privacy.ephemeral": "Aucun prompt enregistré",
  "onboarding.tour.privacy.ephemeralDetail":
    "Le texte existe seulement le temps de la reformulation.",
  "onboarding.tour.privacy.keychain": "Clés protégées par macOS",
  "onboarding.tour.privacy.keychainDetail": "Elles ne sont jamais renvoyées à l'interface.",
  "onboarding.tour.privacy.available": "Toujours à portée de raccourci",
  "onboarding.tour.privacy.availableDetail":
    "La barre des menus garde Reqraft accessible sans Dock.",
  "shortcut.cmd": "Cmd",
  "shortcut.ctrl": "Ctrl",
  "shortcut.option": "Option",
  "shortcut.shift": "Maj",
  "settings.providersConfigured": "{count} provider configuré · télémétrie désactivée",
  "settings.providersConfiguredPlural": "{count} providers configurés · télémétrie désactivée",
  "profiles.levelMeta": "niveau {level}",
  "profiles.confirmDeleteQuestion": "Supprimer « {id} » ? Cette action est définitive.",
  "settings.confirmDeleteKey":
    "Supprimer la clé {provider} du trousseau ? Elle sera définitivement perdue : le trousseau n'a pas de corbeille, et une clé API ne se réaffiche pas chez le fournisseur. Il faudra en générer une nouvelle.",
  "picker.availableOne": "{count} disponible",
  "capsule.replaceUnavailableWhy":
    "Remplacement indisponible ({reason}) — résultat copié, ⌘V pour coller.",
  "capsule.replaceFailedWhy": "Le remplacement a échoué ({reason}) — le résultat reste affiché.",
  "capsule.inventionDetected": "! absent de votre demande",
  "capsule.inventedPaths": "chemins que vous n'avez pas cités : {list}",
  "capsule.inventedCommands": "commandes que vous n'avez pas demandées : {list}",
  "capsule.technicalTermsMissing": "! termes techniques perdus",
  "capsule.missingTechnicalTermsLabel": "absents du résultat :",
  "capsule.missingTechnicalTerms": "absents du résultat : {list}",
  "capsule.structuralInflation": "revenue sous forme de plan",
  "capsule.restructured": "! restructurée",
  "capsule.rewriteInstruction": "! consigne ajoutée",
  "capsule.rewriteInstructionDetail": "le modèle demande de reformuler au lieu de le faire",
  "onboarding.secureStorageUnavailable": SECURE_STORAGE_UNAVAILABLE,
  "main.errorSecureStorageUnavailable": SECURE_STORAGE_UNAVAILABLE,
} as const;
